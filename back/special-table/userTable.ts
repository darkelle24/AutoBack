...
export class UserTableClass<M extends Model> extends TableClass<M> {
  ...
  constructor(auth: userTableConfig, name: string, table: saveTable, sequelizeData: ModelCtor<M>, server: any, filePath: string, originRoutePath?: string) {
    ...
    super(name, table, sequelizeData, server, filePath, originRoutePath)
    this.config = {
